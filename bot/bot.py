...
class Bot(object):
    ...
    def __init__(self, app_configs: Dict, configs: Dict, logger=None):
        ...
        self.debug_info = (configs["debug"], configs["debugPrefix"])
        self.load_mods()
    ...
    def addDebugInfo(self, msg: MessageChain) -> MessageChain:
        if not self.debug_info[0]:
            return msg
        new_msg = MessageChain.create([Plain(self.debug_info[1])])
        new_msg.plus(msg)
        return new_msg
    ...
    async def sendMessage(self, subject: Union[Tuple[str, int], Group, Member,
        ...
        if isinstance(subject, Member):
            ...
            new_msg.plus(msg)
            new_msg = self.addDebugInfo(new_msg)
            await self.app.sendGroupMessage(subject.group, new_msg)
        msg = self.addDebugInfo(msg)
